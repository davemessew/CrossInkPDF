...
PdfStatus encodeHeader(const uint16_t sectionIndex, const uint32_t firstGlobalWordOrdinal,
                       const uint32_t sectionWordCount, uint8_t output[PDF_LAYOUT_WORD_INDEX_HEADER_BYTES]) {
...
PdfStatus decodeHeader(const uint8_t input[PDF_LAYOUT_WORD_INDEX_HEADER_BYTES], PdfLayoutWordIndexInfo* const info) {
  if (std::memcmp(input, kHeaderMagic, sizeof(kHeaderMagic)) != 0 || getU16(input + 4) != kVersion ||
      ...
      getU16(input + 10) != PDF_LAYOUT_WORD_INDEX_RECORD_BYTES || getU32(input + 20) != 0 || getU32(input + 24) != 0 ||
      getU32(input + 28) != pdfCacheCrc32(input, 28)) {
...
PdfStatus encodeRange(const PdfLayoutWordRange& range, const uint32_t cursor,
...
PdfStatus decodeRange(const uint8_t input[PDF_LAYOUT_WORD_INDEX_RECORD_BYTES], PdfLayoutWordRange* const range,
  ...
  if (getU32(input + 24) != 0 || getU32(input + 28) != pdfCacheCrc32(input, 28) ||
...
PdfStatus PdfLayoutWordIndexWriter::begin(const PdfByteSink destination, const uint16_t sectionIndex,
  ...
  *this = {};
  ...
  uint8_t header[PDF_LAYOUT_WORD_INDEX_HEADER_BYTES];
  PdfStatus status = encodeHeader(sectionIndex, firstGlobalWordOrdinal, sectionWordCount, header);
  if (status) {
...
PdfStatus PdfLayoutWordIndexWriter::append(const PdfLayoutWordRange& range) {
  ...
  PdfStatus status = encodeRange(range, nextGlobalWordOrdinal_, encoded);
  if (status) {
  ...
  aggregateCrc_ = pdfCacheCrc32(encoded, PDF_LAYOUT_WORD_INDEX_RECORD_BYTES - sizeof(uint32_t), aggregateCrc_);
  if (range.valid) {
...
PdfStatus pdfInspectLayoutWordIndex(const PdfByteSource& source, PdfLayoutWordIndexInfo* const info) {
  ...
  for (uint16_t firstPage = 0; firstPage < info->pageCount;) {
    ...
    for (uint16_t index = 0; index < count; ++index) {
      ...
      uint32_t cursor = 0;
      status = decodeRange(record, &range, &cursor);
      if (!status) {
        return status;
      }
      aggregateCrc = pdfCacheCrc32(record, PDF_LAYOUT_WORD_INDEX_RECORD_BYTES - sizeof(uint32_t), aggregateCrc);
