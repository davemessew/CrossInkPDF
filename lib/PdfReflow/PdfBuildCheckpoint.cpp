...
PdfStatus pdfDecodeBuildCheckpoint(const PdfByteSource& source, PdfBuildCheckpoint* const checkpoint) {
  ...
  uint8_t reserved8 = 0;
  uint16_t identityReserved = 0;
  uint8_t phase = 0;
  uint8_t phaseReserved8 = 0;
  ...
  *checkpoint = {};
  ...
  PDF_CACHE_RETURN_IF_ERROR(decoder.u16(&identityReserved));
  ...
  PDF_CACHE_RETURN_IF_ERROR(decoder.u8(&phaseReserved8));
  ...
  if (std::memcmp(magic, kCheckpointMagic, sizeof(magic)) != 0 || codecVersion != PDF_CACHE_CODEC_VERSION ||
      reserved16 != 0 || modificationTimeKnown > 1 || reserved8 != 0 || identityReserved != 0 ||
      phase > static_cast<uint8_t>(PdfBuildPhase::Cancelled) || phaseReserved8 != 0 || phaseReserved16 != 0 ||
